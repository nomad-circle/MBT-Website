/* Approved source copy and interface labels. Add a real inquiry endpoint before accepting submissions. */
window.MBT_CONFIG = {
  "team": [
    {
      "name": {
        "en": "Meshal Radhi Al-Mutairi",
        "ar": "مشعل راضي المطيري"
      },
      "role": {
        "en": "Chief Executive Officer (CEO)",
        "ar": "الرئيس التنفيذي"
      },
      "photo": null,
      "bio": {
        "en": "Meshal brings more than 27 years of experience at the Saudi Ministry of Defense, spanning team leadership, project supervision, and government coordination. His background in executive administration and institutional communications supports his leadership of MBT and its relationships across the defense sector.",
        "ar": "يمتلك مشعل خبرة تزيد على 27 عامًا في وزارة الدفاع السعودية، تشمل قيادة فرق العمل والإشراف على المشاريع والتنسيق مع الجهات الحكومية. ويستند في قيادته لشركة MBT إلى خبرته في الإدارة التنفيذية والاتصالات الإدارية وبناء العلاقات المؤسسية في القطاع الدفاعي."
      }
    },
    {
      "name": {
        "en": "Hassan Alfahadi",
        "ar": "حسن الفهادي"
      },
      "role": {
        "en": "Chief Operating Officer (COO)",
        "ar": "الرئيس التنفيذي للعمليات"
      },
      "photo": null,
      "bio": {
        "en": "Hassan brings more than 25 years of experience in operations and management advisory across the public and private sectors. His expertise spans operational strategy, organizational development, and major project delivery, with a focus on improving performance, strengthening governance, and supporting strategic transformation aligned with Saudi Vision 2030.",
        "ar": "يمتلك حسن خبرة تتجاوز 25 عامًا في العمليات والاستشارات الإدارية في القطاعين الحكومي والخاص. وتشمل خبراته الاستراتيجية التشغيلية والتطوير التنظيمي وتنفيذ المشاريع الكبرى، مع التركيز على تحسين الأداء وتعزيز الحوكمة ودعم التحول الاستراتيجي بما يتوافق مع رؤية السعودية 2030."
      }
    },
    {
      "name": {
        "en": "Wahaj Ul Samad",
        "ar": "وهج الصمد"
      },
      "role": {
        "en": "Chief Financial Officer (CFO)",
        "ar": "الرئيس التنفيذي للشؤون المالية"
      },
      "photo": null,
      "bio": {
        "en": "Wahaj brings more than 15 years of experience in financial services and FinTech, spanning financial analysis, risk management, and business advisory. His expertise includes financial planning, budgeting, and regulatory compliance, connecting financial insight with business objectives to support sound decisions and sustainable growth.",
        "ar": "يمتلك وهج خبرة تزيد على 15 عامًا في الخدمات المالية والتقنية المالية، تشمل التحليل المالي وإدارة المخاطر والاستشارات الإدارية. وتتضمن خبراته التخطيط المالي وإعداد الميزانيات والامتثال التنظيمي، بما يدعم اتخاذ قرارات مالية مدروسة وتحقيق نمو مستدام."
      }
    },
    {
      "name": {
        "en": "Khodor Hammoud",
        "ar": "خضر حمود"
      },
      "role": {
        "en": "Business Director",
        "ar": "مدير الأعمال"
      },
      "photo": null,
      "bio": {
        "en": "Khodor brings more than nine years of experience in the Saudi defense and security sector, specializing in business development, strategic partnerships, and market entry. He has worked closely with leading U.S. and European defense manufacturers, supporting commercial opportunities and localization initiatives aligned with Saudi Vision 2030.",
        "ar": "يمتلك خضر خبرة تزيد على تسع سنوات في قطاع الدفاع والأمن السعودي، ويتخصص في تطوير الأعمال والشراكات الاستراتيجية ودخول السوق. عمل مع مصنّعين دفاعيين أمريكيين وأوروبيين رائدين، لدعم الفرص التجارية ومبادرات التوطين المتوافقة مع رؤية السعودية 2030."
      }
    }
  ],
  "contactEmail": "Mohammad@mbt-saudi.com",
  "linkedin": "",
  "inquiryEndpoint": ""
};
window.MBT_CONTENT = {
  "en": {
    "contact": "Partner with MBT",
    "language": "العربية",
    "menu": "Menu",
    "close": "Close menu",
    "skip": "Skip to content",
    "heroLabel": "SAUDI ARABIA / DEFENSE & SECURITY",
    "scroll": "Discover MBT",
    "pillars": [
      "International representation",
      "Market access",
      "Localization",
      "In-country support"
    ],
    "portrait": "Portrait space",
    "formTitle": "Tell us about your interest",
    "fields": [
      "Full name",
      "Work email",
      "Company",
      "Area of interest",
      "Your message"
    ],
    "interest": [
      "Choose an area",
      "Saudi representation",
      "Market development",
      "Localization",
      "Industrial partnerships",
      "Corporate inquiries"
    ],
    "location": "Riyadh, Saudi Arabia",
    "back": "Back to top",
    "rights": "All rights reserved.",
    "inquiryTitle": "MBT Partnership Inquiry",
    "submit": "Send Inquiry",
    "sending": "Sending your inquiry…",
    "sent": "Your inquiry has been sent successfully.",
    "sendUnavailable": "Sending is currently unavailable. Please try again later.",
    "emailPrepared": "Complete sending your inquiry in your email application."
  },
  "ar": {
    "contact": "ابدأ شراكة مع MBT",
    "language": "English",
    "menu": "القائمة",
    "close": "إغلاق القائمة",
    "skip": "انتقل إلى المحتوى",
    "heroLabel": "المملكة العربية السعودية / الدفاع والأمن",
    "scroll": "اكتشف MBT",
    "pillars": [
      "التمثيل الدولي",
      "دخول السوق",
      "التوطين",
      "الدعم داخل المملكة"
    ],
    "portrait": "مساحة الصورة الشخصية",
    "formTitle": "كيف يمكننا مساعدتك؟",
    "fields": [
      "الاسم الكامل",
      "البريد الإلكتروني للعمل",
      "الشركة",
      "مجال الاهتمام",
      "رسالتك"
    ],
    "interest": [
      "اختر المجال",
      "التمثيل في السعودية",
      "تطوير السوق",
      "التوطين",
      "الشراكات الصناعية",
      "استفسارات عامة"
    ],
    "location": "الرياض، المملكة العربية السعودية",
    "back": "العودة إلى الأعلى",
    "rights": "جميع الحقوق محفوظة.",
    "inquiryTitle": "استفسار شراكة مع MBT",
    "submit": "إرسال استفسار",
    "sending": "جارٍ إرسال الاستفسار…",
    "sent": "تم إرسال استفسارك بنجاح.",
    "sendUnavailable": "الإرسال غير متاح حاليًا. يرجى المحاولة لاحقًا.",
    "emailPrepared": "أكمل إرسال استفسارك في تطبيق البريد."
  }
};
